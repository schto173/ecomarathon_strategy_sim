...
class Track:
    ...
    ux: np.ndarray           # unit heading per segment, easting component (toward East)
    uy: np.ndarray           # unit heading per segment, northing component (toward North)
...
def load_track(csv_path: str, a_lat_max: float, v_max: float,
    ...
    # Per-segment heading as a unit vector in the UTM plane (east, north). Used to
    # resolve a steady wind into a per-segment tail/head-wind component.
    dx_h = x[nxt] - x
    dy_h = y[nxt] - y
    seg_len = np.hypot(dx_h, dy_h)
    ...
    return Track(s=s, x=x, y=y, lon=lon, lat=lat, elev=elev, elev_s=elev_s,
                 ds=ds, sin_theta=sin_theta, cos_theta=cos_theta, ux=ux, uy=uy,
                 kappa=kappa, v_cap_point=v_cap_point, v_cap_seg=v_cap_seg,
